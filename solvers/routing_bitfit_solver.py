...
class RoutingBitFitSolver(object):
    ...
    def run(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            for name, param in self.model.named_parameters():
                if 'fix' not in name and ('bias' in name or 'classifier' in name or 'pool' in name): 
            ...
            for batch in tqdm(pos_train_loader):
                ...
                if epoch >= self.args.warm_up:
                    ...
                    self.model.bert.encoder.inference_mode = False
                ...
                if epoch >= self.args.warm_up:
                    ...
                    self.model.bert.encoder.inference_mode = True
                    # no_routing_outputs = self.model(input_ids, attention_mask=attention_mask)
                    # no_routing_logits = no_routing_outputs.logits

                    teacher_preds = (ensemble_preds[indexes]/temperature).softmax(dim=-1)
                    # distillation_loss = ((teacher_preds.detach() - (no_routing_logits/temperature).softmax(dim=-1)) ** 2).mean(dim=-1) * 3 # (-teacher_preds.detach() * (no_routing_logits/temperature).log_softmax(dim=-1)).sum(dim=-1) * (temperature) ** 2
